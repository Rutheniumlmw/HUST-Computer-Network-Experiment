...
void TCPReceiver::receive(const Packet& packet) {
	...
	if (checkSum == packet.checksum && this->expectSequenceNumberRcvd == packet.seqnum) {
		...
		fprintf(TCP_rcver_info, "接收方正确收到发送方的报文 seqnum = %d\n", packet.seqnum);
		//取出Message，向上递交给应用层
		Message msg;
		...
		fprintf(TCP_rcver_info, "接收方发送确认报文 acknum=%d\n", lastAckPkt.acknum);
		...
		if (checkSum != packet.checksum) {
			...
			fprintf(TCP_rcver_info, "接收方没有正确收到发送方的报文,数据校验错误\n");
		}
		else {
			pUtils->printPacket("接收方没有正确收到发送方的报文,报文序号不对", packet);
			fprintf(TCP_rcver_info, "接收方没有正确收到发送方的报文,报文序号不对\n");
		}
