...
void SRReceiver::receive(const Packet& packet) {
	...
	if (checkSum == packet.checksum && offseqnum < this->winlen && window.at(offseqnum).flag == false) {
		...
		fprintf(SR_rcver_info, "接收方 base=%d, offseqnum=%d\n", this->base, offseqnum);
		//将该窗口状态设置为true，占用
		fprintf(SR_rcver_info, "接收方窗口:[ ");
		...
		fprintf(SR_rcver_info, "接收方正确收到发送方的报文\n");
		...
		fprintf(SR_rcver_info, "接收方滑动后窗口:[ ");
		for (int i = 0; i < (signed)window.size(); i++) {
			if (window.at(i).flag == true)
		...
		fprintf(SR_rcver_info, "接收方发送确认报文\n");
		...
		if (checkSum != packet.checksum) {
			...
			fprintf(SR_rcver_info, "接收方没有正确收到发送方的报文,数据校验错误\n");
		}
		else {
			pUtils->printPacket("接收方没有正确收到发送方的报文,报文已确认", packet);
			fprintf(SR_rcver_info, "接收方没有正确收到发送方的报文,报文已确认\n");
			lastAckPkt.acknum = packet.seqnum; //确认序号等于收到的报文序号
			lastAckPkt.checksum = pUtils->calculateCheckSum(lastAckPkt);
			pUtils->printPacket("接收方重新发送上次的确认报文", lastAckPkt);
			fprintf(SR_rcver_info, "接收方重新发送上次的确认报文\n");
